...
1. The number of rows and columns are mutated by adding and/or removing a line
   ...
   and columns are added in the same way as the crossover and :ref:`creation
   <create-ind>` processes respectively. Note that the number of rows and
   columns will not mutate beyond the bounds passed to the genetic algorithm in
   the :code:`row_limits` and :code:`col_limits` parameters.
